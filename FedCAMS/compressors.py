...
class CompressorType:
    ...
    NATURAL_DITHERING_FP32 = (
        8  # Natural Dithering applied for FP32 components vectors
    )
    NATURAL_DITHERING_FP64 = (
        9  # Natural Dithering applied for FP64 components vectors
    )
...
class Compressor:
    ...
    def name(self):
        ...
        if self.compressorType == CompressorType.SIGN_COMPRESSOR:
            return "Sign"
        if self.compressorType == CompressorType.ONEBIT_SIGN_COMPRESSOR:
            return "One Bit Sign"
        return "?"
    ...
    def makeStandardDitheringFP64(
        self, levels, vectorNormCompressor, p=float("inf")
    ):
    ...
    def makeStandardDitheringFP32(
        self, levels, vectorNormCompressor, p=float("inf")
    ):
        self.compressorType = CompressorType.STANDARD_DITHERING_FP32
        self.levelsValues = torch.arange(
            0.0, 1.1, 1.0 / levels
        )  # levels + 1 values in range [0.0, 1.0] which uniformly split this segment
        self.s = len(self.levelsValues) - 1  # should be equal to level
        assert self.s == levels
    ...
    def makeNaturalDitheringFP64(self, levels, dInput, p=float("inf")):
        ...
        r = min(p, 2)
        self.w = 1.0 / 8.0 + (dInput ** (1.0 / r)) / (2 ** (self.s - 1)) * min(
    ...
    def compressVector(self, x, iteration=0):
        ...
        if self.compressorType == CompressorType.IDENTICAL:
            ...
            self.last_need_to_send_advance = (
                2 * top_size * self.component_bits_size
            )
