...
class LocalUpdate(object):
    ...
    def update_weights_local(self, model, global_round):
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), lr=self.args.local_lr, momentum=0
        )
    ...
    def compressSignal(self, signal, D):
        ...
        for t in range(len(signal)):
            ...
            signal_flatten[
                (signal_offset) : (signal_offset + offset)
            ] = signal[t].flatten(0)
        ...
        for t in range(len(signal)):
            offset = len(signal[t].flatten(0))
            signal_compressed[t].flatten(0)[:] = signal_flatten[
                (signal_offset) : (signal_offset + offset)
            ]
            signal_offset += offset
...
def update_model_inplace(
    ...
    iteration = (
        cur_iter + 1
    )  # add 1 is to make sure nonzero denominator in adam calculation
    ...
    for i, param in enumerate(model.parameters()):
        ...
        if args.optimizer == "fedavg":
            ...
            param.data.add_(param.data, alpha=-1).add_(
                par_before[i], alpha=1
            ).add_(grad, alpha=args.lr * lr_decay)
        ...
        # adam calculation
            ...
            exp_avg_sq.mul_(args.beta2).addcmul_(
                grad, grad.conj(), value=1 - args.beta2
            )
            ...
            param.data.add_(param.data, alpha=-1).add_(
                ...
                grad,
                exp_avg_sq.sqrt().add_(args.eps),
                value=args.lr * lr_decay,
            )
