...
def gen_dataset(dataset, shape_img, data_path):
    if dataset == "mnist":
        ...
        trainset = datasets.MNIST(
            root=data_path + "mnist", train=True, download=True, transform=tt
        )
        testset = datasets.MNIST(
            root=data_path + "mnist", train=False, download=True, transform=tt
        )
        ...
        root = data_path + "/Object/ILSVRC/2012/"
        tt_train = transforms.Compose(
            [
                transforms.RandomHorizontalFlip(),
        ...
        testset = datasets.ImageNet(root=root, split="val", transform=tt_tst)
...
def split_noniid_data(dataset, num_users):
    ...
    try:
        ...
        all_labels = np.array(dataset.labs)
    ...
    for i in range(num_users):
        rand_set = set(
            np.random.choice(idx_shard, num_shard_to_choose, replace=False)
        )
    ...
    for file in process_bar:
        ...
        for idx, name in enumerate(names):
            ...
            process_bar.set_postfix(
                file=file, progress=f"{idx + 1}/{len(names)}"
            )
        tar.close()
