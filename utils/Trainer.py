...
def trainer(
    model, train_loader, optimizer, criterion, batchsize, client_key=None
):
    train_epoch_loss = []
    train_epoch_acc = []
    train_bar_obj = tqdm(
    ...
    for gt_data, gt_label in train_bar_obj:
        ...
        train_bar_obj.set_postfix(
            loss=np.mean(train_epoch_loss), accuracy=np.mean(train_epoch_acc)
        )
